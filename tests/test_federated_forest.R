...
local({
  ...
  prepared <- load_federated_no_msk_forest(path, within_site = TRUE)
  stopifnot(nrow(prepared) == 8L, identical(prepared$q_value, rep(d$q_value[1:4], 2)))
  for (site in unique(prepared$site_name)) {
    p <- plot_federated_no_msk_forest(prepared, 0, site_name = site)
    stopifnot(all(na.omit(p$data$site_name) == site), nrow(p$data) == 8L,
              grepl(federated_site_label(site), p$labels$title, fixed = TRUE))
    withCallingHandlers(invisible(ggplotGrob(p)), warning = function(w) stop(w))
  }
  stopifnot(inherits(tryCatch(plot_federated_no_msk_forest(prepared, 0), error = identity), "error"))
  for (bad in list(bind_rows(within, within[1, ]), mutate(within, site_name = "msk_caia_1_1"))) {
    write_csv(bad, path)
    stopifnot(inherits(tryCatch(load_federated_no_msk_forest(path, within_site = TRUE), error = identity), "error"))
  }
})
local({
  ...
  withCallingHandlers(stopifnot(inherits(plot_federated_no_msk_sites(all_counts), "gtable")),
  ...
  withCallingHandlers(invisible(plot_federated_no_msk_sites(filter(all_counts, landmark_days == 90))),
                      warning = function(w) stop(w))
