...
DEFAULT_MAPPING_CSV = PACKAGE_DIR / "resources" / "lab_mappings" / "OMOP_to_DFCI_lab_ids.csv"
DEFAULT_UNIQUE_LABS_CSV = Path("unique_lab_ids_w_units.csv")
...
PHYSIOLOGIC_RANGES: dict[str, tuple[float, float]] = {
    ...
    "Body temperature": (25.0, 45.0),      # °C — Fahrenheit values are
                                           # converted to Celsius first by the
                                           # Body temperature special case in
                                           # convert_measurement_value (e.g. 98.6°F
                                           # becomes 37°C), so they pass this range
                                           # check and are retained.
...
def refresh_mapping_dataframe(
    ...
    for _, row in mapping_df.iterrows():
        ...
        if excluded_count:
            generated_notes.append("Rows with unsupported or semantically incompatible units are excluded by dfci_labs.py.")
...
def _consolidate_dfci_labs_rowwise(
    ...
    for _, row in output_df.iterrows():
        ...
        # Attempt combined blood-pressure detection/splitting BEFORE the
        # exact/prefix mapping lookup treats a "/"-containing value as a scalar.
        # A combined "120/80" whose TEST_NAME collides with a mapped entry would
        # otherwise be coerced to NaN by pd.to_numeric and lost. Only when parsing
        # actually succeeds do we split; genuinely non-BP rows fall through to the
        # normal mapping path below.
        if is_combined_blood_pressure_name(row["TEST_NAME"]):
            ...
            if bp_pair is not None:
                ...
                for bp_component, collapsed_measurement, bp_value in split_specs:
                    ...
                    if bp_metadata is None:
                        ...
                        # Parsing succeeded and the split value is a valid, usable
                        # mmHg reading even though the target measurement is not in
                        # the mapping; mark it as a mapped-class success so
                        # --successful-only does not drop it.
            ...
            # Parsing failed. If the name maps normally, let it flow through the
            # standard mapping path below; only an otherwise-unmapped combined-BP
            # name gets the requires_bp_split marker.
            if metadata is None:
        ...
        # Per-measurement physiologic range filter; bounds are in canonical
        # units. Out-of-range values get their standardized value nulled with a
        # status code so the row stays in the audit trail. Sentinel values were
        # already removed at numeric coercion; this catches plausible-looking
        # but physiologically impossible entries.
