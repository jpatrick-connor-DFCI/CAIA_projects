...
Note: COMPASS also builds a wide person-period `longitudinal_landmark{D}.csv`
for its Dynamic DeepHit / XGBoost arms. That builder is hard-wired to COMPASS's
two-endpoint PLATINUM/DEATH competing-risk columns (t_platinum, t_death, ...)
and there is no dynamic-DeepHit/XGBoost work in scope for IPIO yet, so it is
deliberately NOT ported here -- see the comment near the bottom of `main()`.
The priority for IPIO is `aggregated_landmark{D}.csv`, consumed by
cox_univariate.py / cox_multivariable.py.
"""
...
# COMPASS's helpers.cohort / helpers.helper are schema-agnostic (configurable
...
# Not helpers.loaders.load_profile_longitudinal -- that loader's patient-level
...
def load_ipio_longitudinal(path: Path, *, id_col: str = "DFCI_MRN") -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    helpers.loaders.load_profile_longitudinal handles a missing t_lab).
    """
...
# ---------------------------------------------------------------------------
# IPIO-local equivalent of helpers.cohort.build_landmark_merged: that function
...
# itself is reused unchanged from helpers.cohort.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Single-endpoint stratification (IPIO has only IRAE; COMPASS's
# choose_stratification_labels looks for PLATINUM/DEATH/EITHER, none of which
# exist here, so it would silently fall back to "unstratified").
# ---------------------------------------------------------------------------

def choose_stratification_labels(df: pd.DataFrame, *, min_count: int) -> tuple[np.ndarray | None, str]:
    if "IRAE" in df.columns:
        labels = pd.to_numeric(df["IRAE"], errors="coerce").fillna(0).astype(int).to_numpy()
...
def main(args: argparse.Namespace) -> None:
    ...
    for landmark_day in landmark_days:
        ...
        # NOTE: COMPASS also builds a wide person-period longitudinal_landmark{D}.csv
        # here (for Dynamic DeepHit / XGBoost). That builder is hard-wired to the
        # two-endpoint PLATINUM/DEATH competing-risk columns and there is no
        # dynamic-DeepHit/XGBoost work in scope for IPIO yet, so it is intentionally
        # NOT ported -- adapting it purely to keep an unused code path "working"
        # risked introducing bugs into the aggregated-table build, which is the
        # priority (consumed directly by cox_univariate.py / cox_multivariable.py).
