...
Two sweeps per endpoint:
  ...
  2) Genomic indicators (auto-detected gene x variant-type indicators, e.g.
     {gene}_{SV,DEL,AMP,SNV}):
...
def run_genomic_indicator_univariate(
    ...
    *,
    ...
    """Cox on [AGE + static_covariate_cols + indicator] per binary genomic feature, per endpoint."""
    require_lifelines()
    ...
    for feature in feature_cols:
        ...
        if feature not in data.columns:
            ...
            rows.append(result)
            continue
        missing_static = [c for c in static_covariate_cols if c not in data.columns]
        if missing_static:
            result["note"] = f"missing_static_covariate_columns({','.join(missing_static)})"
            rows.append(result)
            continue
        keep_cols = [AGE_COL, *static_covariate_cols, feature, duration_col, event_col]
...
def main(args: argparse.Namespace) -> None:
    ...
    static_covariate_cols: tuple[str, ...] = tuple(_ca.baseline_covariate_columns(aggregated))

    # Genomic indicator columns are auto-detected (pan-cancer somatic file: all
    # genes present), rather than a fixed gene panel.
    ...
    for endpoint in endpoints:
        ...
        # Lab features (n_obs-adjusted), always including baseline covariates.
