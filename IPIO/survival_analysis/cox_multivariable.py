...
--baseline: fit an age(+baseline-covariate)-only Cox model (no lab features,
  no CV / feature selection) on the same horizon grid for benchmarking. Reuses
  the multivariable final-fit + evaluation path so the metrics schema matches.
...
def _run_multivariable_landmark(
    ...
    *,
    ...
    static_covariate_cols = tuple(_ca.baseline_covariate_columns(ctx.merged))
...
def _run_baseline_landmark(
    ...
    *,
    ...
    """Age(+baseline-covariate)-only baseline arm for one landmark; appends to ``out`` lists.

    No lab / genomic features, no CV / feature selection. Reuses the
    multivariable final-fit + evaluation path so C-index / AUC(t) / Brier are
    on the identical horizon grid and the metrics CSV schema matches the
    multivariable arm.
    """
    static_covariate_cols = tuple(_ca.baseline_covariate_columns(ctx.merged))
    ...
    print("\n##### BASELINE: AGE(+BASELINE COVARIATES)-ONLY (all endpoints) #####")
    print(
        "  covariates: age"
...
def main(args: argparse.Namespace) -> None:
    ...
    # / _load_prebuilt_landmark read the right id/age columns.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Multivariable elastic-net Cox (+ age(+baseline covariates) baseline) on landmarked lab features."
    )
    parser.add_argument(
        "--id-col",
        default=ID_COL,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Fit an age(+baseline-covariates)-only Cox model (no labs, no CV) on the "
            "same horizon grid for benchmarking, instead of the elastic-net lab model."
        ),
    )
    parser.add_argument(
        "--auc-max-time-units",
