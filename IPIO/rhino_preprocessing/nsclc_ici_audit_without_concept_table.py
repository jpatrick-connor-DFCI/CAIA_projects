...
spark.sql("""
CREATE OR REPLACE TEMP VIEW temp_nsclc_concepts AS
SELECT concept_id
FROM (VALUES
...
# ---------------------------------------------------------------
# Part 2: Identify all NSCLC patients and label ICI status
# ---------------------------------------------------------------
spark.sql("USE CATALOG dfci_ia_aistudio")
spark.sql("USE SCHEMA omop_caia_denorm")

# All NSCLC patients with earliest diagnosis date
spark.sql("""
CREATE OR REPLACE TEMP VIEW nsclc_patients AS
SELECT
    co.person_id,
    MIN(co.condition_start_date) AS nsclc_diagnosis_date
FROM dn_condition_occurrence_20251219 co
WHERE co.condition_concept_id IN (SELECT concept_id FROM temp_nsclc_concepts)
GROUP BY co.person_id
""")

# Patients with any ICI exposure
spark.sql("""
CREATE OR REPLACE TEMP VIEW ici_patients AS
...
# ---------------------------------------------------------------
total = spark.sql("SELECT COUNT(*) AS n FROM nsclc_patients").collect()[0]["n"]
print(f"Total NSCLC patients in dataset: {total:,}")
print()

print("=" * 80)
print("NSCLC PATIENTS BY ICI STATUS")
print("=" * 80)
spark.sql(f"""
    SELECT
        ici_status,
    ...
    FROM nsclc_ici_labeled
    GROUP BY ici_status
    ORDER BY n_patients DESC
""").show(truncate=False)

print("=" * 80)
print("NSCLC DIAGNOSIS YEAR BY ICI STATUS")
print("=" * 80)
spark.sql("""
    SELECT
        ici_status,
        YEAR(nsclc_diagnosis_date) AS dx_year,
        COUNT(*) AS n_patients
    FROM nsclc_ici_labeled
    GROUP BY ici_status, YEAR(nsclc_diagnosis_date)
    ORDER BY ici_status, dx_year
