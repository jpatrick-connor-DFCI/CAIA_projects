...
SELECT descendant_concept_id AS concept_id, 'NSCLC'   AS cancer_type FROM concept_ancestor WHERE ancestor_concept_id IN (
  -- List of NSCLC condition concept IDs
...
JOIN concept c
  ON ca.ancestor_concept_id = c.concept_id
WHERE c.vocabulary_id = 'ATC'
  AND c.concept_code = 'H02AB'
""")

# === 7. Lab measurement concepts (FIX: added 3008893 and 3013603) ===
spark.sql("""
...
spark.table(CONCEPT_TABLE).cache().createOrReplaceTempView("concept_cached")
CONCEPT_TABLE = "concept_cached"
...
JOIN dn_measurement_20251219 m
    ...
    AND m.measurement_concept_id IN (
      ...
      3023103,3020630,3034426,3024171,3019550,3004249,3008893,3013682,3013466,
      3013603,
      3002131,3034548,3000285,3000483,3004295,3005456,3018572,
