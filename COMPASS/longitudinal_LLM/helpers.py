"""Shared plumbing for the longitudinal (per-note) LLM extraction pipelines.

Both pipelines (Gleason timeline, AVPC/NEPC criteria timeline) make ONE LLM call
per trigger-bearing note, then aggregate the per-note extractions into a
patient-level timeline. The generic LLM client / retry / JSON / note-loading
helpers are reused from the NEPC classifier; this module adds the per-note
snippet iteration and small extraction utilities those pipelines share.
"""
...
COMPASS_DIR = CURRENT_DIR.parent
if str(COMPASS_DIR) not in sys.path:
    sys.path.insert(0, str(COMPASS_DIR))

# Reused, trigger-agnostic plumbing from the NEPC classifier.
from NEPC_classifier.helpers import (  # noqa: E402,F401
