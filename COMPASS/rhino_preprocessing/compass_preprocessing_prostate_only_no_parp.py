# ===============================================================
# COMPASS – Prostate-Only / No-PARP Variant
# ===============================================================
# This is a restricted version of compass_preprocessing.py that additionally:
#   1. Excludes patients with ANY non-prostate malignant neoplasm diagnosis
#   2. Excludes patients who ever received a PARP inhibitor
# All other logic (lab extraction, unit conversion, etc.) is identical.
# ===============================================================
...
# Fully-qualified reference for concept lookups later
CONCEPT_TABLE = "snowflake_aistudio_full_catalog.omop_cdm_deid.concept"

# Cache the concept table once — it's referenced 6+ times across the pipeline
spark.table(CONCEPT_TABLE).cache().createOrReplaceTempView("concept")
CONCEPT_TABLE = "concept"  # redirect all subsequent references to the cached version
...
# === 1b. Non-prostate malignant neoplasm concepts ===
# All descendants of "Malignant neoplastic disease" (443392) MINUS prostate cancer descendants.
# Used to exclude patients with any other cancer diagnosis.
spark.sql("""
...
# === 1c. PARP inhibitor drug concepts ===
# Find ingredient concepts by name, then get all descendant drug concepts.
# Covers: olaparib, rucaparib, niraparib, talazoparib
spark.sql("""
...
WHERE concept_id IN (
  ...
  -- The following 24 PSA variant IDs have no unit data in the CSV and are silently
  -- dropped by the allowed_unit_combinations inner join. Commented out until verified.
...
spark.sql("""
CREATE OR REPLACE TEMP VIEW temp_prostate_psa10_patients_unfiltered AS
SELECT f.person_id
...
# === 7b. Exclude patients with any non-prostate malignant neoplasm ===
spark.sql("""
CREATE OR REPLACE TEMP VIEW temp_patients_with_other_cancer AS
SELECT DISTINCT person_id
FROM dn_condition_occurrence_20251219
WHERE condition_concept_id IN (SELECT concept_id FROM temp_non_prostate_cancer_concepts)
""")

# === 7c. Exclude patients who ever received a PARP inhibitor ===
spark.sql("""
CREATE OR REPLACE TEMP VIEW temp_patients_with_parp AS
SELECT DISTINCT person_id
FROM dn_drug_exposure_20251219
WHERE drug_concept_id IN (SELECT drug_concept_id FROM temp_parp_drugs)
""")

# === 7d. Final filtered cohort: prostate-only, no PARP ===
spark.sql("""
CREATE OR REPLACE TEMP VIEW temp_prostate_psa10_patients AS
SELECT person_id
FROM temp_prostate_psa10_patients_unfiltered
...
n_before_exclusions = spark.table("temp_prostate_psa10_patients_unfiltered").count()
n_with_other_cancer = spark.sql("""
    SELECT COUNT(*) FROM temp_prostate_psa10_patients_unfiltered
    WHERE person_id IN (SELECT person_id FROM temp_patients_with_other_cancer)
""").collect()[0][0]
n_with_parp = spark.sql("""
    SELECT COUNT(*) FROM temp_prostate_psa10_patients_unfiltered
    WHERE person_id IN (SELECT person_id FROM temp_patients_with_parp)
""").collect()[0][0]
n_with_both = spark.sql("""
    SELECT COUNT(*) FROM temp_prostate_psa10_patients_unfiltered
    WHERE person_id IN (SELECT person_id FROM temp_patients_with_other_cancer)
      AND person_id IN (SELECT person_id FROM temp_patients_with_parp)
""").collect()[0][0]

print(f"Patients meeting initial criteria (prostate dx + ≥10 PSA):  {n_before_exclusions:,}")
print(f"  Excluded: non-prostate cancer diagnosis:                   {n_with_other_cancer:,}")
print(f"  Excluded: PARP inhibitor exposure:                         {n_with_parp:,}")
print(f"  Excluded: both:                                            {n_with_both:,}")
print(f"  Total excluded (union):                                    {n_before_exclusions - patient_count:,}")
print(f"Final cohort:                                                {patient_count:,}")
