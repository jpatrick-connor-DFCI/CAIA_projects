...
Used by PROFILE figure-generation notebooks and scripts. Centralises:
  ...
  * `overlay_hist()`      - labelled overlay histograms
  * `overlay_km()`        - labelled Kaplan-Meier overlays
"""
...
# LOINC-style long lab names -> short PROFILE-style names.
# Used by `canonicalize_lab_name()` so CATEGORY_MAP lookups, IRNT panels, and
# overlay comparisons agree on a single label per analyte. Extend as new labs
# appear.
